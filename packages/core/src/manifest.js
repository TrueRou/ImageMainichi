const VALID_RULE_TYPES = ['json-api', 'css-selector', 'rss', 'manhuagui'];
const VALID_MODES = ['crawl', 'on-demand'];
...
function validateRule(rule) {
    ...
    if (!VALID_MODES.includes(r.mode)) {
    ...
    switch (r.type) {
        ...
        case 'css-selector':
            ...
            break;
        case 'rss':
            if (!['enclosure', 'media:content', 'content-img'].includes(r.imageFrom)) {
                throw new Error(`rss rule "${r.name}" requires valid "imageFrom"`);
            }
            break;
        case 'manhuagui':
            if (!['latest-chapter', 'all-chapters'].includes(r.scope)) {
